...
**PHASE 1 COMPLETE — Sprints 0–7 merged to `main` (9e31315).** The full MVP works end-to-end against the cloud dev backend: auth → create room (server-generated rotatable code) → join by code (rate-limited, owner-approved) → role-scoped access → evidence vault (sha256 chain-of-custody, auto-versioning, dual-layer permission denials) → timeline/discussion/tasks with realtime + @mentions → immutable audit trail auto-mirrored to a human-readable timeline. 75/75 pgTAP RLS contract tests + 59/59 Dart tests, analyze 0, CI green on every merge. The demo-day flow (Phases.md §7 steps 1–3 + populated audit) rehearsed LIVE on cloud dev with zero manual DB intervention. Architecture proof: the Academic case type runs on pure config — adding new domains (Phase 2) needs seed rows, not code. **Standing infra:** Docker Desktop + local stack (DB port 65432; `npx supabase start --exclude studio,imgproxy,inbucket,edge-runtime,logflare,analytics,vector` — the full 12-service start crashed the engine on this machine, start lean); `db reset` now seeds demo data automatically. **Next:** Phase 2 (remaining case types, redaction, notifications, PDF export, entity map) per Phases.md §3 — Go/No-Go below.**Next:** Sprint 7 (final Phase-1 sprint): error-handling audit, seed/demo data, demo-day rehearsal (create → join → upload → populated audit log) on Chrome + Android, Phase-1 retro, Phase-2 Go/No-Go.
