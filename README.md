...
**UX Parity PRD** (`CaseThread-UX-Parity-PRD.md`): client-requested light theme + debug tools, case-room overview enrichment, export overhaul (PDF/TXT/Print), evidence AI analysis + verify-alibi, discussion enhancements, demo seeding + briefing editor, and optional chat media features. Phases 1–7 not yet started; no backend changes unless a phase explicitly says so.

**Migration numbering note:** `0040`/`0041` are already taken by shipped bugfixes — Phase 6 demo seed uses `0042_demo_seed_expansion.sql`, Phase 7.1 chat-media bucket uses `0043`.
