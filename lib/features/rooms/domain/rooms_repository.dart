...
abstract class RoomsRepository {
  ...
  /// Investigation status (Phase 5): current status of the
  /// investigation lifecycle (open/under_investigation/review/closed).
  Future<InvestigationStatus> getInvestigationStatus(String roomId);

  /// Investigation status (Phase 5): transition to a new status.
  /// On →closed the server auto-generates a case-closed summary
  /// (PRD §4.5).
  ...
  /// Case-closed summary (Phase 5): generated by the server when a
  /// room's investigation status transitions to closed.
  Future<CaseClosedSummary?> getClosedSummary(String roomId);

  /// Phase 6: sets the room's shared case briefing (edit_case holders;
  /// the case_rooms UPDATE policy enforces it server-side).
  Future<void> updateBriefing(String roomId, String briefing);
}
